...
class StepPredictionSaver(Subscriber):
    def __init__(self, path: str, period: int, when=None):
        Subscriber.__init__(self, when)
        self.path = Path(path)
        self.path = pipeline_repository.create_dir_if_not_exist(self.path)
        self.period = period
    
    def update(self, prediction, iteration, target, input, norm_mean, norm_std, **kwargs):
        if iteration % self.period != 0: return
        batch_size = len(prediction)
        for k in range(batch_size):
            img, pred, label = input[k], prediction[k], target[k]
            renormed_img = renorm_tensor(img, norm_mean, norm_std).type(torch.int8)
            renormed_img = to_pil_image(img)
            pred, label = to_pil_image(pred.float()), to_pil_image(label.float())
            image = h_concatenate_images(pred, label)
            image = h_concatenate_images(renormed_img, image.convert("RGB"))
            img_path = self.path / f"{iteration}.png"
            image.save(img_path)
