...
def _split_data(input_dir: Path, test_ratio: float, valid_ratio: float) -> tuple:
    ...
    splits = {}
    for path in input_dir.glob("img-*"):
    ...
    to_mask_path = lambda p: p.parent / f"mask-{p.stem.split('-')[1]}.{p.name.split('.')[1]}"
    for split_name, img_paths in splits.items():
        mask_paths = [to_mask_path(Path(p)) for p in img_paths]
        splits[split_name] += mask_paths
    return splits
...
def normalize_datasets(datasets: dict, aug_dict: dict, tensor_tf_dict: dict):
    ...
    train_mean, train_std = train_dataset.normalize()
    for split_name, dataset in datasets.items():
        ...
        dataset.transforms = Compose.from_composits(split_aug_tf, split_tensor_tf) 
    return datasets
