...
import hr.fer.rasus.components.SensorFactory;
...
public class Main {
		...
		try(Scanner s = new Scanner(System.in)){
			while(true) {
				...
				for(Integer port : ports) {
					...
					UdpClients.sendPacket(sendBuf, port);
				}
	...
	public static void createSensorTable() throws IOException {
		try(PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(SensorFactory.SENSOR_TABLE, false)), true);){
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		final int SENSOR_NUM = 5;
		sensors = SensorFactory.createSensors(SENSOR_NUM);
