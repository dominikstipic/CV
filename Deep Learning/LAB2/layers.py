...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    losses = (y*log_probs).sum(axis=1)
    L = -losses.mean()
    return L

  def softmax(self,x):
      exp_x = np.exp(x-np.max(x))
      exp_sums = np.sum(exp_x, axis=1).reshape(-1,1)
      probs = exp_x/exp_sums
      return probs
...
if __name__ == "__main__":
    ...
    l = SoftmaxCrossEntropyWithLogits()
    
    print(loss(input,target))
    
    input  = input.numpy()
    target = target.numpy() 
    print(l.forward(input,target))
